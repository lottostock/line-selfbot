                ...
                if msg.toType in [0,2]:
                   ...
                   if msg.contentType == 0:
                      ...
                      if cmd== ".customing" or cmd== rname + ".customing":
                          ...
                          #schedule.every(16).seconds.do(lottoyekee)

                          
                      if cmd== ".lotto" or cmd== rname + ".lotto" :
                          lottoyekee()
                          if cmd.startswith('.'):label = cmd.replace('.','')
                          else:label = cmd.replace(rname,"")
                          menu = open('help/lotto.txt','r').read()
                          to = 'ca42c3b4b95237b04b34c2d70b87dbf88'
                          client.help(msg.to,label,menu)  
                          #while True:
                      ...
                      '''  ** UTILITY **  ''' 
                      if cmd== ".res" or cmd== rname + "res":
                            if cmd.startswith('.'):label = cmd.replace('.','')
                            else:label = cmd.replace(rname,"")
                            lotto = open('lotto/lottoresults.txt','r').read()
                            client.sendMessage(msg.to,label,lotto)
                      ...
                      if cmd== ".tagall" or cmd== "@@@":
                         ...
                         for mentionMembers in range(midSelect+1):
                             ...
                             for dataMention in group.members[mentionMembers*20 : (mentionMembers+1)*20]:
                                 ...
                                 ret_ += "\n{}. @!\n".format(str(no)).replace("/0️⃣/gi", "apples")
                                 no = (no+1)
                      ...
                      if cmd== ".ginfo" or cmd== rname + "ginfo":
                          ...
                          if group.pictureStatus is None:gpict = "https://www.img.in.th/images/8d8a98f486d4e22cf0cf0d80340bfc7d.png"
